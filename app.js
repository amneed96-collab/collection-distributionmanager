...
function goTo(view) { VIEW = view; render(); }
...
function viewReps() {
  ...
  return `
    <div class="toolbar">
      ...
      <div><button class="btn-ghost" onclick="openDeliveryModal()" ${!DATA.customers.length || !DATA.reps.length || !DATA.products.length ? "disabled" : ""}>+ Record Delivery</button> <button class="btn-primary" onclick="openRepModal()">+ Add Representative</button></div>
    </div>
    <div class="panel">
...
function viewCollection() {
  ...
  return `
    <div class="toolbar"><div style="font-size:12.5px;color:var(--muted);">Admin কাস্টমার থেকে টাকা কালেকশন করবে।</div>
      <button class="btn-primary" onclick="openCollectionModal()" ${!DATA.customers.length || !DATA.reps.length ? "disabled" : ""}>+ Record Collection</button></div>
    <div class="panel">
      <table class="data-table" id="collectionTable">
        <thead><tr><th>Date</th><th>Customer</th><th>Rep</th><th>Method</th><th>Amount</th><th></th></tr></thead>
        <tbody>
          ${rows.length === 0 ? `<tr><td colspan="6"><div class="empty-state">কোনো কালেকশন নেই।</div></td></tr>` :
            rows.map((c) => `<tr><td>${fmtDate(c.date)}</td><td><strong>${esc(idToName(DATA.customers, c.customerId))}</strong></td><td>${esc(idToName(DATA.reps, c.repId))}</td>
              <td><span class="badge">${esc(c.method)}</span></td><td class="mono-num tone-success-text">${fmtMoney(c.amount)}</td>
              <td><button class="icon-btn" onclick="openCollectionModal('${c.id}')">✏️</button><button class="icon-btn" onclick="deleteCollection('${c.id}')">🗑️</button></td></tr>`).join("")}
        </tbody>
        ${rows.length ? `<tfoot><tr><td colspan="4" style="text-align:right;font-weight:700;">Total</td><td class="mono-num tone-success-text">${fmtMoney(total)}</td><td></td></tr></tfoot>` : ""}
      </table>
    </div>`;
}
...
function viewDistribution() {
  ...
  return `
    <div class="toolbar"><div style="font-size:12.5px;color:var(--muted);">Admin কালেকশন করা টাকা প্রতিনিধি ও কোম্পানীকে ভাগ করে দেবে।</div>
      <button class="btn-primary" onclick="openDistributionModal()" ${!DATA.reps.length || !DATA.companies.length ? "disabled" : ""}>+ Record Distribution</button></div>
    <div class="grid" style="margin-bottom:16px;">
    ...
    <div class="panel">
      <table class="data-table" id="distributionTable">
        <thead><tr><th>Date</th><th>Rep</th><th>Company</th><th>To Rep</th><th>To Company</th><th>Total</th><th></th></tr></thead>
        <tbody>
          ${rows.length === 0 ? `<tr><td colspan="7"><div class="empty-state">কোনো ডিস্ট্রিবিউশন নেই।</div></td></tr>` :
            rows.map((x) => `<tr><td>${fmtDate(x.date)}</td><td><strong>${esc(idToName(DATA.reps, x.repId))}</strong></td><td>${esc(idToName(DATA.companies, x.companyId))}</td>
              ...
              <td><button class="icon-btn" onclick="openDistributionModal('${x.id}')">✏️</button><button class="icon-btn" onclick="deleteDistribution('${x.id}')">🗑️</button></td></tr>`).join("")}
        </tbody>
      </table>
    </div>`;
}
...
function customerLedgerHTML() {
  ...
  return `
    ...
    <div class="panel"><h3 class="panel-title">Ledger — ${esc(customer.name)}</h3>
...
function repLedgerHTML() {
  ...
  return `
    ...
    <div class="panel"><h3 class="panel-title">Ledger — ${esc(rep.name)}</h3>
...
function reportDaily() {
  ...
  return `
    <input class="field-input" type="date" style="max-width:200px;margin-bottom:14px;" value="${REPORT_DAY}" onchange="REPORT_DAY=this.value;renderView();">
    <div class="grid" style="margin-bottom:16px;">
...
function reportMonthly() {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (col || del || dist) rows.push({ d, col, del, dist });
  }
  return `
    <input class="field-input" type="month" style="max-width:200px;margin-bottom:14px;" value="${REPORT_MONTH}" onchange="REPORT_MONTH=this.value;renderView();">
    <div class="grid" style="margin-bottom:16px;">
...
function reportYearly() {
  ...
  return `
    <input class="field-input" type="number" style="max-width:120px;margin-bottom:14px;" value="${REPORT_YEAR}" onchange="REPORT_YEAR=this.value;renderView();">
    <div class="grid" style="margin-bottom:16px;">
      <div class="stat-card"><div class="stat-label">Collections</div><div class="stat-value tone-success-text">${fmtMoney(totalCol)}</div></div>
      <div class="stat-card"><div class="stat-label">Deliveries</div><div class="stat-value tone-gold-text">${fmtMoney(totalDel)}</div></div>
      <div class="stat-card"><div class="stat-label">Distributed</div><div class="stat-value">${fmtMoney(totalDist)}</div></div>
